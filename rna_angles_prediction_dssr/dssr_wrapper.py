...
class DSSRWrapper:
    ...
    def setup_env_variables(self):
        ...
        base_name = os.path.dirname(
            os.path.dirname(self.dssr_analyse_bin_path))
    ...
    def get_all_angles_from_dssr_one_file(
        ...
        df_pseudo_torsion = self.get_angles_from_dssr_one_file(
            file_, is_pseudo=True)
...
def dssr_outputer(data_path, output_path):
    all_files = [name for name in os.listdir(
        data_path) if name.endswith(".pdb")]
    for _pdb_name in all_files:
        pdb_path = os.path.join(data_path, _pdb_name)
        current_output_path = os.path.join(
            output_path, _pdb_name.replace(".pdb", ".csv"))
        dssr_wrapper.run(pdb_path, current_output_path, to_csv=True)
        print(f"Done for {pdb_path} and {current_output_path}")
